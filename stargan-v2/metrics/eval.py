...
from core.data_loader import get_eval_loader, get_test_style_loader
    ...
    for trg_idx, trg_domain in enumerate(domains):
        ...
        if mode == 'reference':
            if args.norm_type == 'adain':
                ...
                loader_ref = get_test_style_loader(root=args.val_img_dir, domain=trg_domain,
                                                   num_embeds=num_embeds,
                                                   batch_size=args.val_batch_size,
                                                   num_workers=args.num_workers)
            else:
                raise NotImplementedError('Normalization type [%s] is not found' % args.norm_type)
        ...
        for src_idx, src_domain in enumerate(src_domains):
            ...
            for i, x_src in enumerate(tqdm(loader_src, total=len(loader_src))):
                ...
                for j in range(args.num_outs_per_domain):
                    if mode == 'latent':
                        nets.feature_extractor.module.set_std_weight(2)
                        nets.feature_extractor.module.inference_running_stats = True
                        if args.norm_type == 'adain':
                            z_trg = torch.randn(N, args.latent_dim).to(device)
                            s_trg = nets.mapping_network(z_trg, y_trg)
                        elif args.norm_type == 'sean':
                            z_trg = torch.randn(N, args.hidden_nc).to(device)
                        ...
                        s_trg = get_style_code(nets, args.norm_type, num_embeds, y_trg, x_ref, z_trg=None)
                        x_fake = nets.generator(x_src, s_trg, labels=y_trg, masks=masks)
